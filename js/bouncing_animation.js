...
function animateScene(gl, programInfo, buffers, toWorld) {
    ...
    function step(timestamp) {
        ...
        var projectionMatrix = gl.getUniform(programInfo.program, programInfo.uniformLocations.projectionMatrix);
        mat4.invert(projectionMatrix, projectionMatrix);

        // Calculate boundaries
        var min = vec4.fromValues(-1.0, -1.0, -0.02, 1.0);
        ...
        // Calculate boundaries
        var max = vec4.fromValues(1.0, 1.0, -0.02, 1.0);
        ...
        mat4.translate(toWorld, toWorld, [tx[0] * delta, tx[1] * delta, tx[2] * delta]);
...
function drawScene(gl, programInfo, buffers, toWorld) {
    ...
    {
        const offset = 0;
        const vertexCount = gl.getBufferParameter(gl.ARRAY_BUFFER, gl.BUFFER_SIZE);
